...
class CompetitionFilterService {
  ...
  static List<Season> filterSeasons(Event event, List<Season> seasons) {
    return seasons.where((season) => !_isSeasonExcluded(event, season)).toList();
  }
  
  /// Filter divisions for a specific event and season based on configuration exclusions
  static List<Division> filterDivisions(Event event, String season, List<Division> divisions) {
    return divisions.where((division) => !_isDivisionExcluded(event, season, division)).toList();
  }
  ...
  static bool _isDivisionExcluded(Event event, String season, Division division) {
    // First check if the event or season is excluded
    final seasonObj = Season(title: season, slug: season);
    if (_isEventExcluded(event) || _isSeasonExcluded(event, seasonObj)) return true;
  ...
  static List<String> get excludedDivisionCombos => _config.excludedDivisionCombos;
}